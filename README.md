# Wikipedia Newsletter ETL Project
...
- ETL pipelines
    ...
    - Loading the data to the databases that the final user will fetch
...
So the idea was the following: "I want to create an ETL that will retrieve the HTML page of Wikipedia in its corresponding day, get the list of five events that happened that day and store it in a database. This will repeat every day".
Once retrieved, I thought about two options: send it to my email or display it in a webpage.
Sending it to the email would force me to read it, which would make me learn a lot of things at the end of the week. I would say this project is something like a "Wikipedia Daily Newsletter".

I could have done any of those two ideas, but at the end I got just to the "storing to a database" stage, since I didn't want to have my AWS jobs Active 24/7. I wanted to see if I could manage to finish this project and run it at least once!
...
## Concepts learned
In this section, I wanted to write about other concepts learned in this project:

**3 Vs (Verity, velocity, and volume)**
In short, volume refers to the amount of data, variety refers to the number of types of data and velocity refers to the speed of data processing. According to the 3Vs model, the challenges of big data management result from the expansion of all three properties, rather than just the volume alone.

Using AWS, we have the advantage of requiring minimal maintenance regarding these three properties. Cloud Service Providers will auto-scale for us and eventually lead to less maintenance and cost.
However, this doesn't free us of keeping an optimal pipeline and retrieving what is neccessary.
In this stage, it is important to consider the analytics part and understand if what we are retrieving is what we really need.

In this project we don't need to worry about these concepts, but it is good to keep them in mind.

---

**HA (High Availability)**
A Cloud Databases HA instance group includes a source database instance and one or two replicas. If main fails, other will take the place until main is restored. It is important to create plan B, C, D in case something bad happens. We have to consider all possible failure scenarios. AWS facilitates snapshots, but we have to be proactive and do constant backups of our data (enough data to fix something if it recently failed, but not too much data that would conflict with the 3 Vs (volume in particular). Keep data as long as needed, but not more)
...
In this project, I am doing five inserts in a table each day (one for each of the events displayed). A good method to deal with Atomicity would be to make sure that the five insertions have been completed successfully. We don't want to have this task completed at half.
...
**Logs**
Logs should be collected in the following situations:
...
Logs help us to find the cause of failure of our system, saving time in investigation. "They are not needed until they are".
In order to write them, we need to consider: who will be the audience, which format should they have and taht we should keep them as long as needed, but not more

## Conclusion
A summary, during the development of this project I learned a lot of new concepts that were unknown to me before. I am happy that I managed to complete the project and that I learned about AWS (which I was looking forward for some time). Their free tier allows to build almost anything you can think about, so I will start thinking about the next project I want to do! I am sure it will be something more complex and challenging since I now have the basis of Data Engineering!
